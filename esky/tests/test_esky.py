...
class TestEsky(unittest.TestCase):
    ...
    if py2app is not None:
        ...
        if esky.sudo.can_get_root():
            ...
            def test_esky_py2app_needsroot(self):
                with setenv("ESKY_NEEDSROOT", "1"):
                    self._run_eskytester(
                        {"bdist_esky": {"freezer_module": "py2app"}})
    ...
    if cx_Freeze is not None:
        ...
        def test_esky_cxfreeze(self):
            self._run_eskytester(
                {"bdist_esky": {"freezer_module": "cxfreeze"}})
...
#    os.mkdir(tdir)
        ...
        try:
            ...
            options["bdist_esky"].setdefault(
                "excludes", []).extend(["Tkinter", "tkinter"])
            options["bdist_esky"]["compress"] = "ZIP"
            platform = get_platform()
            deploydir = "deploy.%s" % (platform,)
            esky_root = dirname(dirname(dirname(__file__)))
            os.chdir(tdir)
            shutil.copytree(os.path.join(esky_root, "esky",
                                         "tests", "eskytester"), "eskytester")
            ...
            script2 = Executable(
                [None, open("eskytester/script2.py")], name="script2")
            script3 = "eskytester/script3.py"
            dist_setup(version="0.1", scripts=[script1], options=options, script_args=[
                       "bdist_esky"], **metadata)
            dist_setup(version="0.2", scripts=[
                       script1, script2], options=options2, script_args=["bdist_esky"], **metadata)
            dist_setup(version="0.3", scripts=[script2, script3], options=options, script_args=[
                       "bdist_esky_patch"], **metadata)
            os.unlink(os.path.join(tdir, "dist",
                                   "eskytester-0.3.%s.zip" % (platform,)))
            #  Check that the patches apply cleanly
            uzdir = os.path.join(tdir, "unzip")
            deep_extract_zipfile(os.path.join(
                tdir, "dist", "eskytester-0.1.%s.zip" % (platform,)), uzdir)
            with open(os.path.join(tdir, "dist", "eskytester-0.3.%s.from-0.1.patch" % (platform,)), "rb") as f:
                esky.patch.apply_patch(uzdir, f)
            really_rmtree(uzdir)
            deep_extract_zipfile(os.path.join(
                tdir, "dist", "eskytester-0.2.%s.zip" % (platform,)), uzdir)
            ...
            zfname = os.path.join(
                tdir, "dist", "eskytester-0.1.%s.zip" % (platform,))
            os.mkdir(deploydir)
    ...
    def test_esky_lock_breaking(self):
        ...
        appdir = tempfile.mkdtemp()
        try:
            os.makedirs(os.path.join(appdir, ESKY_APPDATA_DIR,
                                     "testapp-0.1", ESKY_CONTROL_DIR))
            open(os.path.join(appdir, ESKY_APPDATA_DIR, "testapp-0.1",
                              ESKY_CONTROL_DIR, "bootstrap-manifest.txt"), "wb").close()
            e1 = esky.Esky(appdir, "http://example.com/downloads/")
            e2 = esky.Esky(appdir, "http://example.com/downloads/")
            trigger1 = threading.Event()
            trigger2 = threading.Event()
...
class TestPatch(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.workdir = workdir = os.path.join(
            self.tests_root, "patch-test-temp." + platform)
    ...
    def test_patch_bigfile(self):
        ...
        try:
            ...
            with open(os.path.join(tdir, "patch"), "wb") as f:
                esky.patch.write_patch(os.path.join(
                    tdir, "source"), os.path.join(tdir, "target"), f)
    ...
    def test_apply_patch_old(self):
        ...
        path1, path2 = self._extract(
            "pyenchant-1.2.0.tar.gz", "pyenchant-1.6.0.tar.gz")
        ...
        if not os.path.exists(pf):
            pf = os.path.join(dirname(esky.__file__), "tests",
                              "patch-test-files", "v1.2.0_to_v1.6.0.patch")
